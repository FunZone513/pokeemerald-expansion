...
static u8 ChooseWildMonIndex_Fishing(u8 rod)
{
    ...
    case OLD_ROD:
        ...
        if (rand >= ENCOUNTER_CHANCE_FISHING_MONS_OLD_ROD_SLOT_1 && rand < ENCOUNTER_CHANCE_FISHING_MONS_OLD_ROD_SLOT_2)
            wildMonIndex = 1;
        if (rand >= ENCOUNTER_CHANCE_FISHING_MONS_OLD_ROD_SLOT_3 && rand < ENCOUNTER_CHANCE_FISHING_MONS_OLD_ROD_SLOT_3)
            wildMonIndex = 3;
...
bool8 TryDoDoubleWildBattle(void)
{
    if (GetSafariZoneFlag()
      || (B_DOUBLE_WILD_REQUIRE_2_MONS == TRUE && GetMonsStateToDoubles() != PLAYER_HAS_TWO_USABLE_MONS))
        ...
        return TRUE;
    else if (VarGet(VAR_WILD_BATTLE_CHANCE) != 0 && ((Random() % 100) + 1 <= VarGet(VAR_WILD_BATTLE_CHANCE)))
        return TRUE;
    return FALSE;
}
