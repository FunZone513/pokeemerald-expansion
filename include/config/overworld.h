...
#define OW_PC_HEAL                  GEN_5 // In Gen8+, Pokémon are not healed when deposited in the PC.
...
#define B_FLAG_FOLLOWERS_DISABLED      FLAG_ENABLE_FOLLOWER_POKEMON          // Enables / Disables followers by using a flag. Helpful to disable followers for a period of time.
...
#define OW_FLAG_POKE_RIDER          FLAG_FLY_FROM_NAV  // If this flag is set, the player will be able to use fly from the Pokenav Region Map and the Town Map key item by pressing 'R' on a city/location they are able to fly to.
